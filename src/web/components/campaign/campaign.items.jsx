...
const CampaignItems = ({
  ...
  const [state, setState] = useState({
    modal: false,
    addedItems: selectedItems.reduce((obj, item) => {
      obj[item.Id] = item;
      return obj;
    }, {}),
  });
  const openModal = () => {
    dispatch({
      ...
      payload: campaign.Organization.Id,
      params,
    });
  ...
  const onSelect = (item) => {
    ...
    if (find) {
      dispatch({
        type: "REMOVE_CAMPAIGN_ITEMS_START",
        payload: { campaignId: campaign.Id, itemId: item.Id },
      });
    } else {
      dispatch({
        type: "ADD_CAMPAIGN_ITEMS_START",
        payload: { Campaign: campaign, Item: item },
      });
    }
  };
  ...
  const mappedItems = Object.keys(state.addedItems).map((key) => {
    return { ...state.addedItems[key] };
  });
  const columns = [
    {
      name: "Name",
...
const mapState = (state) => {
  ...
  return {
    ...
    itemsCountPerPage:
      item && item.itemsCountPerPage ? item.itemsCountPerPage : 0,
    pageRangeDisplayed:
      item && item.pageRangeDisplayed ? item.pageRangeDisplayed : 0,
  };
};
