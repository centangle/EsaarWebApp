...
import {
  ...
  addRequestThreadSuccess,
  addRequestThreadFailure,
...
export function* fetchThreadAsync(action) {
  const currentUser = yield select(selectCurrentUser);
  const q =
    "recordsPerPage=0&type=General&currentPage=1&orderDir=Desc&disablePagination=true&entityType=Organization&entityId=" +
    action.payload;
  try {
...
export function* fetchRequestStatusAsync() {
  ...
  try {
    const response = yield fetch(
      url + "/api/OrganizationRequest/GetRequestStatus",
      {
        method: "GET",
        //withCredentials: true,
        credentials: "include",
...
export function* updateRequestAsync(action) {
  try {
    ...
    const request = yield fetch(
      url + "/api/Request/UpdateMultipleRequestsWithChildrens",
      {
        method: "PUT",
        //withCredentials: true,
        headers: {
...
export function* modifyRegionsAsync(action) {
  try {
    const currentUser = yield select(selectCurrentUser);
    const request = yield fetch(
      url + "/api/OrganizationMember/UpdateOrganizationMembershipRegions",
      {
        method: "POST",
        //withCredentials: true,
        headers: {
...
export function* assignRequestAsync(action) {
  try {
    ...
    const request = yield fetch(
      url + "/api/OrganizationRequest/AssignRequest?" + q,
      {
        method: "PUT",
        //withCredentials: true,
        headers: {
...
export function* fetchRequestDetailsAsync(action) {
  ...
  try {
    const response = yield fetch(
      url + "/api/OrganizationRequest/Get?requestId=" + action.payload,
      {
        method: "GET",
        //withCredentials: true,
        credentials: "include",
...
export function* fetchRequestStatus() {
  yield takeLatest(
    requestTypes.FETCH_REQUEST_STATUS_START,
...
export function* fetchThreadDetail() {
  yield takeLatest(
    requestTypes.FETCH_ORG_THREAD_DETAIL_START,
    fetchThreadDetailAsync
  );
}
export function* fetchRequestDetaail() {
  yield takeLatest(
    requestTypes.FETCH_ORG_REQUEST_DETAIL_START,
    fetchRequestDetailsAsync
  );
}
